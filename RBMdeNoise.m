%Main program but de-noise variant

function RBMdeNoise(gpuID,prop) %Input gpu and options to run RBM
...
%% Calculate a gibbs sample THIS IS WHERE DIFFERENCE TO RBM IS
...
model.h = gpuArray(pHj>120/255);    %Hidden layer activation as cutoff (Heaviside)
model.h = model.h.*dOut';
pVj = gpuArray(1./(1+arrayfun(@(x)exp(-x), net.a + net.W'*model.h)));
model.v = gpuArray(pVj>120/255);    %Visible layer activation as cutoff (Heaviside)
end    
net.gibbsSample = model.v; 
end
